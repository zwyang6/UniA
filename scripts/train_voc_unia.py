...
torch.hub.set_dir("/data/PROJECTS/SeCo_2024/cvpr_2024_SeCo/SeCo_代码/SeCo_原始实验代码/RoCo_SOTA_Visualization/pretrained")
...
parser.add_argument("--w_ptc", default=0.0, type=float, help="w_ptc")
parser.add_argument("--w_kl", default=0.1, type=float, help="w_ctc")
parser.add_argument("--w_aff", default=0.0, type=float, help="w_ptc")
...
parser.add_argument("--aux2final", default=8000, type=int, help="use dense refined masks to refine aff")
...
def validate(model=None, data_loader=None, args=None):
    ...
    cam_aff_score = evaluate.scores(gts, cams_aff)
    cam_dr_score = evaluate.scores(gts, cams_dr)
    model.train()

    tab_results = format_tabs([cam_score, cam_aux_score, cam_aff_score, cam_dr_score, seg_score], name_list=["CAM", "aux_CAM", "aff_Map", "dr_Map", "Seg_Pred"], cat_list=voc.class_list)
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        aff_label = final_seg_label if n_iter >= args.aux2final else pseudo_label_aux
        aff_label = cams_to_affinity_label(pseudo_label_aux, mask=attn_mask, ignore_index=args.ignore_index)
        aff_loss = get_aff_loss(attn_pred, aff_label)

        # kl loss
        KL_loss = KLloss(reduction='batchmean')
        # confident_label =  final_seg_label
        confident_label =  final_seg_label if n_iter >= args.aux2final else pseudo_label_aux.clone().float()
