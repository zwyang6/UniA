...
sys.path.append("/data/ziqing/Jaye_Files2/unicam/unicam/")
...
from model.model_seg_neg_mct_mask_cle_vae import network
...
parser.add_argument("--model_path", default="/data/ziqing/Jaye_Files2/unicam/unicam/w_outputs/2024-04/unia_vae_seed0_11-10-25-47/checkpoints/model_iter_18000.pth", type=str, help="model_path")
...
def _validate(model=None, data_loader=None, args=None):
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            #! TO DO
            cls_x4, seg, _x4, cls_aux, attn_pred, uncer_out = model(inputs,)
            un_map = F.interpolate(uncer_out, size=labels.shape[1:], mode='bilinear', align_corners=False).max(1).cpu().numpy()
            un_map = un_map/(un_map.max() + 1e-6) * 255 
        
            imageio.imsave(args.segs_dir + "/" + name[0] + ".png", (np.squeeze(un_map).cpu().numpy().astype(np.uint8)))
...
if __name__ == "__main__":
    ...
    base_dir = '/data/ziqing/Jaye_Files2/unicam/unicam/uncertainty_test/'
