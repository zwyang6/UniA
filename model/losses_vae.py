...
class KLloss(nn.Module):
    ...
    def forward(self, input, target, log_var=None, mu=None):
        b,h,w = input.shape
        ...
        kl_fit = F.kl_div(input, target, reduction=self.reduction)

        log_var, mu = F.normalize(log_var, p=2, dim=-1, eps=1e-8).sum(-1), F.normalize(mu, p=2, dim=-1, eps=1e-8).sum(-1),
        kl_regularize = -0.5*(log_var+1-mu**2-torch.exp(log_var)).sum() / b

        return 0.9*kl_fit + 0.1*kl_regularize
